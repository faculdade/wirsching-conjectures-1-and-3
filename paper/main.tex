...
$\liminf$ over $\ell$ to a bound holding for every $\ell\ge\ell_0$. Its
quantifiers are worth writing out, because they are stronger than
$(\star2)$'s in a way that matters below. Wirsching's Theorem 2 reads
``if there are real numbers $\delta,\mu>0$ and an index $\ell_0$ such
that\ldots'' \cite[\S3]{Wirsching2003}, so all three are chosen before
$a$:
\[
 \exists\,\delta,\mu>0\ \ \exists\,\ell_0\ \ \forall a\in\mathbb Z_3^\times\ \
 \forall\ell\ge\ell_0\ \ \forall(x_\ell)\in\widetilde A_\delta:\quad
 \widetilde g_\ell(x_\ell,a)\ \ge\ \mu\int_{\mathbb Z_3^\times}
 \widetilde g_\ell(x_\ell,b)\,db .
\]
In $(\star1)$ and $(\star2)$ the eventual index sits inside a
$\liminf$ and may depend on $a$. Here it may not: one $\ell_0$ serves
every unit at once. That is the extra demand $(\star3)$ makes, and
\S\ref{sec:discussion} returns to it;
...
The quantifier order matters here. Wirsching states each of
$(\star1)$, $(\star2)$ and $(\star3)$ as ``if there are real numbers
$\delta,\mu>0$ such that\ldots'', with $a$ occurring free inside the
displayed inequality \cite[\S\S1--3]{Wirsching2003}; he does not write
the quantifier on $a$ out. It is read universally, which places
$\delta_1$ and $\mu_1$ before it and makes them global:
\[
...
$\ell_0$ may depend on $a$; the constant $\mu_1$ may not. That
distinction is load bearing, and it is the reading Wirsching's own
proof of Theorem 1 uses, where a single positive constant has to serve
every admissible $a$ to yield uniform predecessor density. A $\mu_1$
free to depend on $a$ and tend to $0$ would leave nothing uniform to
conclude. Below,
...
statement is what is used, and it is what those memoirs prove.
Hadamard sets it as his own object, pp.~199--200: ``il n'a m\^eme pas
\'et\'e \'etabli que la fonction $\zeta$ n'ait pas de z\'eros sur la
droite $\mathfrak{R}(s)=1$. C'est cette derni\`ere conclusion que je me
propose de d\'emontrer.'' De la Vall\'ee Poussin proves it
independently and, in an addendum, records that Hadamard ``d\'emontre,
lui aussi, que $\zeta(s)$ n'a pas de z\'eros sur la droite
$\mathfrak{R}(s)=1$''. No quantitative zero-free region is needed
here.}, so
...
\S\ref{sec:chain}, and it has two parts. An implication whose
hypothesis never sees $a$ cannot on its own produce a bound uniform in
$a$; and $(\star3)$ asks for more than $(\star1)$ and $(\star2)$ do,
since its index $\ell_0$ is quantified before $a$ and so must serve
every unit at once, where theirs sits inside a $\liminf$ and may vary
with $a$. Neither bridge the source offers from $(\star4)$ toward
$(\star3)$ is uniform in growing resolution. Wirsching's own analytic route of two years
